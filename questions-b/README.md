...
* Конструкторы и наследование: слайсинг, `using`, виртуальные конструкторы и паттерн "фабричная функция" (в том числе для `make_unique` и копирования)
...
* Placement new (практика `17-210211`), выравнивание (`alignas`/`alignof`), явный вызов деструктора, время жизни объектов (lifetime) и недопустимость операций за пределами lifetime,
  свои реализации `std::optional` и `std::vector`.
