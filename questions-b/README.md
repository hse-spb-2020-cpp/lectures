...
### Сети
* Использование `boost::asio::ip::tcp::iostream` на сервере и на клиенте
* Отличия `local_endpoint()` от `remote_endpoint()`

### Базовая многопоточность
* Создание потоков в C++11, передача аргументов в функцию потока
...
* Условные переменные: как использовать, что защищают
* Spurious wakeup
* Реализация producer-consumer
* Реализация `promise`/`future` (без `shared_future`)
* Реализация `latch`

### Дизайн многопоточных приложений
* Deadlock, reentrant-функции, `recursive_mutex`
